...
import { BadgeItem } from "../Ether Menu/Profile.jsx";
...
export default function EditProfile({ user }) {
  ...
  const deleteBadge = useCallback((key) => {
    setUserdata((prev) => ({
      ...prev,
      equippedbadges: prev.equippedbadges.filter((_, i) => i !== key),
    }));
  }, []);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const path = `https://3000-idx-ethersbackend-1741062191902.cluster-nx3nmmkbnfe54q3dd4pfbgilpc.cloudworkstations.dev/api/users/profile/edit/${user}`;
      ...
      try {
        ...
        if (!isCancelled) {
          if (
            ...
            data.data[0].story?.characters == null ||
            data.data[0].story?.dialogues == null
          ) {
            let x = data.data[0];
            x.story = {
              title: "",
        ...
        <div className="PEPContent">
          ...
          <div className="PEPGrid">
            {equippedbadges?.map((element, i) => (
              <div key={i} onClick={() => deleteBadge(i)}>
                <BadgeItem
                  name={element.badgename}
                  description={element.badgedescription}
                  c={"var(--SG)"}
...
const PECInputTextArea = React.memo(
  ({ fieldValue, fieldName, updatedColumn, updateUserData, limit }) => {
...
const ControllerStory = React.memo(
  ({
    avatars,
    story,
    updateUserStory,
    ...
    const [latestID, setLatestID] = useState(
      story.dialogues[story.dialogues?.length - 1]?.dialogueid || 1
    );
                      ...
                      <motion.div className="PECBioItemImg" layout>
                        <AnimatePresence>
                          <motion.img
                            ...
                            exit={{ x: "-100%" }}
                            key={
                              story?.characters[story?.dialogues[i]?.character]
                                ?.tokenid
                            }
                        ...
                        >
                          ...
                          <div className="DialogueInputTrigger">
                            <div
                              ...
                              onClick={() => updateLeft(i, element.left)}
                              style={
                                story?.dialogues[i]?.left == true
...
const StoryHeaders = React.memo(
  ({ title, description, updateStorytitle, modifyStoryDescription }) => {
    return (
      <div className="SectionWrapper">
        <div className="SectionHeader">
          <svg
      ...
      <div className="SectionWrapper">
        <div className="CharacterSelection">
          <div className="SectionHeader">
            ...
            <p>
              ({characters.filter((v) => v !== null && v !== undefined).length}
              /8 Slots filled)
            </p>
          </div>
